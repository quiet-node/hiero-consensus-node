 ...
 */
public class CollectionPcesWriter {
    private static final Logger logger = LogManager.getLogger(CollectionPcesWriter.class);
    ...
    public record PrepareResult(
            List<PlatformEvent> eventsToWrite, boolean closeAfterWriting, List<PlatformEvent> remainingEvents) {}

    /**
     * Prepare the output stream for a particular event. May create a new file/stream if needed.
     *
     * @param events the event that is about to be written
     * @return true if this method call resulted in the current file being closed
     */
    public PrepareResult prepareOutputStream(@NonNull final List<PlatformEvent> events) throws IOException {
        ...
        if (currentMutableFile == null) {
            final long upperBound = nonAncientBoundary
                    + computeNewFileSpan(nonAncientBoundary, fileType.selectIndicator(events.getLast()));
        ...
        // TODO, we should implement this logic
        return new PrepareResult(events, false, List.of());
    }
