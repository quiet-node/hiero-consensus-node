...
public class FreezeUpgradeCommand implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        ...
        if (delegate.getFinalSpecs().getFirst().getStatus() == HapiSpec.SpecStatus.PASSED) {
            config.output().info("SUCCESS - NMT software upgrade in motion from " + upgradeFile + " artifacts ZIP");
        } else {
            config.output().warn("FAILED - NMT software upgrade is not in motion ");
            return 1;
        }
