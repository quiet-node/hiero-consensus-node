...
class ScheduleServiceApiProviderTest {
    private static final long CONSENSUS_SECOND = 123L;
    ...
    void basicsAsExpected() {
        ...
        assertInstanceOf(
                ScheduleServiceApiProvider.Impl.class,
    ...
    void emptyThrottleImpliesNoCapacity() {
        ...
        assertFalse(impl.hasContractCallCapacity(CONSENSUS_SECOND, GAS_LIMIT, AccountID.DEFAULT));
    }
    ...
    void hasCapacityIfThrottleAllows() {
        ...
        given(scheduleCreateHandler.loadThrottle(
                        any(), eq(DEFAULT_CONFIG.getConfigData(SchedulingConfig.class)), eq(123L)))
                .willReturn(Optional.of(throttle));
        given(throttle.allow(AccountID.DEFAULT, SYNTH_BODY, CONTRACT_CALL, Instant.ofEpochSecond(CONSENSUS_SECOND)))
                .willReturn(true);

        assertTrue(impl.hasContractCallCapacity(CONSENSUS_SECOND, GAS_LIMIT, AccountID.DEFAULT));
    }
