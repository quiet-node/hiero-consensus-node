...
public class ParallelProcessingUtil {
    public final static ForkJoinPool VALIDATOR_FORK_JOIN_POOL;
    ...
    public static ForkJoinTask<?> processRange(long start, long end, LongConsumer consumer) {
        return VALIDATOR_FORK_JOIN_POOL.submit(() -> LongStream.range(start, end).parallel().forEach(consumer));
    }

    public static <T> ForkJoinTask<?> processObjects(List<T> objectsToProcess, Consumer<T> consumer) {
        return VALIDATOR_FORK_JOIN_POOL.submit(() -> objectsToProcess.stream().parallel().forEach(consumer));
    }
